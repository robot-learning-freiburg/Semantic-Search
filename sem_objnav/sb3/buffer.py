...
from sem_objnav.sb3.type_aliases import DictRolloutBufferSamples
...
class DictRolloutBuffer(RolloutBuffer):
    ...
    def reset(self) -> None:
        ...
        if self.use_aux:
            self.aux_angle = np.zeros(
                (self.buffer_size, self.n_envs, self.aux_dim), dtype=np.float32
            )
            self.aux_angle_gt = np.zeros(
        ...
        # self.scenes = np.zeros((self.buffer_size, self.n_envs), dtype=object)
        self.generator_ready = False
    ...
    def add(
        ...
        # self.scenes[self.pos] = np.array(scenes).copy()
        self.pos += 1

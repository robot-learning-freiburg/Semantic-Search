...
from nicr_mt_scene_analysis.model.encoder_decoder_fusion import \
...
from nicr_mt_scene_analysis.task_helper.instance import \
    KNOWN_INSTANCE_CENTER_LOSS_FUNCTIONS
from nicr_mt_scene_analysis.task_helper.normal import \
    KNOWN_NORMAL_LOSS_FUNCTIONS

from .data import KNOWN_CLASS_WEIGHTINGS, KNOWN_DATASETS
...
class ArgParserEMSANet(ap.ArgumentParser):
    def __init__(self, *args, **kwargs):
        ...
        group.add_argument("--seed", type=int, default=42, help="Seed for randomness.")
        ...
        group.add_argument(
            "--n-epochs", type=int, default=500, help="Number of epochs to train for."
        )
        group.add_argument(
            "--batch-size", type=int, default=8, help="Batch size to use for training."
        )
        group.add_argument(
            "--optimizer",
            type=str,
        ...
        group.add_argument(
            "--momentum", type=float, default=0.9, help="Momentum to use."
        )
        group.add_argument(
            "--weight-decay",
            type=float,
        ...
        group.add_argument(
            "--semantic-loss-ohem-threshold",
            type=float,
            default=0.0,
            help="Top K threshold to use for online hard example mining in "
            "semantic segmentation. 0.0 disables OHEM.",
        )
        group.add_argument(
            "--semantic-loss-ohem-weighted",
            action="store_true",
            default=False,
            help="Use semantic class weighting while doing OHEM.",
        )
        group.add_argument(
            "--semantic-loss-uncertainity",
            action="store_true",
            default=False,
        )
        group.add_argument(
            "--semantic-loss-lovasz",
            action="store_true",
            default=False,
            help="Add Lovasz loss to semantic segmentation loss.",
        )
        group.add_argument(
        ...
        group.add_argument(
            "--semantic-loss-saturate-unc-loss-epoch",
            type=int,
            default=50,
        )
        group.add_argument(
            "--semantic-loss-uncertainity-coef",
            type=float,
            default=0.03,
        )
        ...
        group.add_argument(
            "--hm3d-semantic-n-classes",
            type=int,
            default=81,
            choices=(81, 53, 33, 37, 8, 40),
            help="Number of semantic classes to use for hm3d dataset.",
        )
        ...
        # -> checkpointing
        group = self.add_argument_group("Validation/Evaluation -> Checkpointing")
        group.add_argument(
    ...
    def parse_args(self, args=None, namespace=None, verbose=True):
        ...
        if "resume" == pa.action:
            ...
            ckpt_fp = os.path.join(pa.resume_path, "checkpoints", "ckpt_resume.pth")
        ...
        if pa.normalization is not None:
            ...
            _warn(
                "Forced `encoder-normalization` and `decoder-normalization`, "
        ...
        if pa.rgb_encoder_backbone_block is not None:
            ...
            _warn(
                "Forced `rgb-encoder-backbone-resnet-block`, to be "
        ...
        if pa.depth_encoder_backbone_block is not None:
            ...
            _warn(
                "Forced `depth-encoder-backbone-resnet-block`, to be "
        ...
        if pa.encoder_decoder_fusion is not None:
            ...
            _warn(
                "Forced `semantic-encoder-decoder-fusion`, to be "
            ...
            _warn(
                "Forced `normal-encoder-decoder-fusion`, to be "
                f"'{pa.encoder_decoder_fusion}' as `encoder-decoder-fusion` "
                "was given."
            )
        ...
        if 1 == len(pa.input_modalities):
            ...
            _warn(
                "Set `encoder-fusion` to 'none' as there is only one input " "modality."
            )
        ...
        if "orientation" in pa.tasks:
            if "instance" not in pa.tasks:
                raise ValueError(
                    "Task 'instance' is missing in `tasks` for "
        ...
        if pa.enable_panoptic:
            if "semantic" not in pa.tasks:
                raise ValueError(
                    "Task 'semantic' is missing in `tasks` for "
        ...
        if pa.batch_size != 8:
            ...
            _warn(
                f"Adapted learning rate to '{pa.learning_rate}' as the "
        ...
        if len(pa.tasks_weighting) != len(pa.tasks):
            raise ValueError(
                "Length for given task weighting does not match "
        ...
        if "coco" in pa.dataset:
            ...
            if "normal" in pa.tasks:
                raise ValueError("COCO dataset does not feature surface " "normals.")
            if "scene" in pa.tasks:
                raise ValueError(
                    "Scene classification is not supported for " "COCO dataset."
                )
        ...
        if pa.semantic_decoder not in decoders_with_ms:
            if not pa.semantic_no_multiscale_supervision:
                ...
                _warn(
                    "Forced `semantic-no-multiscale-supervision` as "
        ...
        if pa.instance_decoder not in decoders_with_ms:
            if not pa.instance_no_multiscale_supervision:
                ...
                _warn(
                    "Forced `instance-no-multiscale-supervision` as "
        ...
        if pa.normal_decoder not in decoders_with_ms:
            if not pa.normal_no_multiscale_supervision:
                ...
                _warn(
                    "Forced `normal-no-multiscale-supervision` as "
        ...
        if pa.validation_batch_size is None:
            ...
            _warn(
                f"`validation-batch-size` not given, using default: "
                f"{pa.validation_batch_size}."
            )

        # handle some common misconfigurations
        if "valid" == pa.validation_split and pa.dataset in ("nyuv2", "sunrgbd"):
            pa.validation_split = "test"
            _warn(
                f"Dataset '{pa.dataset}' does not have a 'valid' split, "
                "using 'test' split instead."
            )

        if pa.validation_skip > pa.checkpointing_skip:
            _warn(
                f"Setting `checkpointing_skip` to '{pa.validation_skip}' as "
        ...
        if pa.visualize_validation:
            if pa.visualization_output_path is None:
                weights_dirpath, weights_filename = os.path.split(pa.weights_filepath)
        ...
        if all(
            ...
            if t_weights_created < t_fix:
                _warn(
                    "Detected Hypersim checkpoint created earlier than "
        ...
        if pa.debug:
            _warn(
                "`debug` is set, enabling debug outputs and ONNX export "
        ...
        if pa.wandb_name is not None:
            _warn(
                "Parameter `wandb-name` is deprecated, use `wandb-project` " "instead."
            )

...
import numpy as np
from nicr_mt_scene_analysis.data import (
    CollateIgnoredDict,
    RandomSamplerSubset,
    mt_collate,
)
...
from nicr_scene_analysis_datasets.pytorch import (
    ConcatDataset,
    DatasetType,
    ScanNet,
    get_dataset_class,
...
def parse_datasets(
    ...
    for i, dataset in enumerate(datasets):
        ...
        re_res = re.findall("([a-z0-9\\_\\-]+)\\[?([a-z0-9\\_\\-]*)\\]?", dataset)
...
def get_dataset(args, split):
    ...
    default_dataset_kwargs = {
        ...
        "hm3d": {
            "semantic_n_classes": args.hm3d_semantic_n_classes,
        },
        "scenenetrgbd": {"depth_mode": dataset_depth_mode, "semantic_n_classes": 13},
        "sunrgbd": {
            "depth_mode": dataset_depth_mode,
    ...
    for i, (dataset_name, dataset) in enumerate(datasets.items()):
        ...
        if "hypersim" == dataset_name and args.hypersim_use_old_depth_stats:
            ...
            assert (
                dataset_instance.depth_std
                == dataset_instance._TRAIN_SPLIT_DEPTH_STATS_V030.std
            )
            assert (
                dataset_instance.depth_mean
                == dataset_instance._TRAIN_SPLIT_DEPTH_STATS_V030.mean
            )
...
class DataHelper:
    ...
    def enable_overfitting_mode(self, n_valid_batches: int) -> None:
        ...
        print(
            "Enable overfitting mode (same data for training and validation) "

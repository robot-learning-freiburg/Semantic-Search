...
from nicr_mt_scene_analysis.model.encoder_decoder_fusion import (
    get_encoder_decoder_fusion_class,
)
from nicr_mt_scene_analysis.model.encoder_fusion import get_encoder_fusion_class
from nicr_mt_scene_analysis.model.initialization import (
    he_initialization,
    zero_residual_initialization,
)
...
class EMSANet(torch.nn.Module):
    def __init__(self, args, dataset_config: DatasetConfig) -> None:
        super().__init__()
        ...
        if "rgb" in args.input_modalities:
            backbone_rgb = get_backbone(
                ...
                resnet_block=get_block_class(
                    args.rgb_encoder_backbone_resnet_block, dropout_p=args.dropout_p
                ),
                n_input_channels=3,
        ...
        if "depth" in args.input_modalities:
            backbone_depth = get_backbone(
                ...
                resnet_block=get_block_class(
                    args.depth_encoder_backbone_resnet_block, dropout_p=args.dropout_p
                ),
                n_input_channels=1,
                normalization=args.encoder_normalization,
                activation=args.activation,
        ...
        if "rgbd" in args.input_modalities:
            backbone_rgbd = get_backbone(
                ...
                resnet_block=get_block_class(
                    args.rgbd_encoder_backbone_resnet_block, dropout_p=args.dropout_p
                ),
        ...
        self.context_module = get_context_module(
            ...
            input_size=(
                args.input_height // enc_downsampling,
        ...
        self.decoders = get_decoders(
            ...
            evidential_output=args.semantic_loss_uncertainity,
        )
        ...
        for part in args.he_init:
            ...
            if "encoder-fusion" == part:
                ...
                cls = get_encoder_decoder_fusion_class(args.encoder_decoder_fusion)
            ...
            if "context-module" == part:
                ...
                he_initialization(
                    self.decoders, blacklist=(Upsampling,), debug=debug_init
                )
    ...
    def forward(self, batch, do_postprocessing=False) -> Dict[str, Any]:
        ...
        for decoder in self.decoders.values():
            outputs.append(
                decoder(
                    (con_outputs, con_context_outputs),
                    enc_dec_skips,
                    batch,

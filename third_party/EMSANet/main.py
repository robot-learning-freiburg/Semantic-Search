...
from emsanet.data import get_datahelper, parse_datasets
...
from emsanet.task_helper import TaskHelperType, get_task_helpers
from emsanet.visualization import setup_shared_color_generators, visualize
from emsanet.weights import load_weights
from nicr_mt_scene_analysis.checkpointing import CheckpointHelper
from nicr_mt_scene_analysis.data import infer_batch_size, move_batch_to_device
from nicr_mt_scene_analysis.logging import CSVLogger
from nicr_mt_scene_analysis.testing.onnx import export_onnx_model
from nicr_mt_scene_analysis.utils import cprint, cprint_step
...
class RunHelper:
    ...
    def training_step(self, batch, batch_idx, epoch_idx):
        ...
        logs = {}
        for task_helper in self._task_helpers:
            task_helper.current_epoch_idx = epoch_idx
    ...
    def training_get_artifacts_and_metrics(self):
        ...
        for key, metric in self._accumulated_step_metrics.items():
            ...
            metric.reset()
        for task_helper in self._task_helpers:
            if hasattr(task_helper, "training_epoch_end"):
                task_helper.training_epoch_end()
    ...
    def validation_get_artifacts_examples_metrics(self):
        ...
        for task_helper in self._task_helpers:
            ...
            metrics.update({f"valid_{key}": value for key, value in task_logs.items()})
            artifacts.update(
                {f"valid_{key}": value for key, value in task_artifacts.items()}
            )
            examples.update(
                {f"valid_{key}": value for key, value in task_examples.items()}
            )
...
def main():
    ...
    csv_logger = CSVLogger(
        filepath=os.path.join(results_path, "metrics.csv"), write_interval=1
    )
    ...
    run = RunHelper(
        args, model=model, task_helpers=task_helpers, device=torch.device("cuda")
    )
    ...
    if args.resume_ckpt_filepath is not None:
        ...
        checkpoint = torch.load(
            args.resume_ckpt_filepath, map_location=torch.device("cpu")
        )
    ...
    checkpoint_helper = CheckpointHelper(
        metric_names=args.checkpointing_metrics, debug=True  # args.debug
    )
    ...
    if not args.skip_sanity_check:
        ...
        batch = next(iter(data.train_dataloader))
        assert isinstance(run.training_step(batch, 0, 0), torch.Tensor)
        ...
        for ckpt_metric in args.checkpointing_metrics or []:
            assert checkpoint_helper._determine_checkpoint_metrics(ckpt_metric, metrics)
    ...
    if args.validation_only:
        ...
        if args.visualize_validation:
            print("Writing visualizations to: " f"'{args.visualization_output_path}'.")
        ...
        run.set_inference_mode()
        batch_idx = 0
        results_path = os.path.join(results_path, "val_results")
        ...
        for i, valid_dataloader in enumerate(data.valid_dataloaders):
            ...
            for batch_id, batch in tqdm(
                enumerate(valid_dataloader), total=len(valid_dataloader), desc=tqdm_desc
            ):
                _, predictions = run.validation_step(batch, batch_idx)
                # val_results
                gt = batch["semantic"].cpu().numpy()
                pred = predictions["semantic_segmentation_idx"].cpu().numpy()
                probs = predictions["semantic_softmax_scores"].cpu().numpy()
                # write gt, pred, prob to results_path as 3 numpy files with "i_batch_id_gt/pred/probs.npy"
                np.save(os.path.join(results_path, f"{batch_id}_gt.npy"), gt)
                np.save(os.path.join(results_path, f"{batch_id}_pred.npy"), pred)
                np.save(os.path.join(results_path, f"{batch_id}_probs.npy"), probs)
                if args.visualize_validation:
                    output_path = os.path.join(
                        args.visualization_output_path, args.validation_split
                    )
                    visualize(
    ...
    try:
        for epoch in range(next_epoch, args.n_epochs):
            cprint(
                f"Epoch: {epoch:04d}/{args.n_epochs-1:04d}",
                color="cyan",
                attrs=("bold",),
            )
            ...
            for batch_idx, batch in tqdm(
                enumerate(data.train_dataloader),
                total=len(data.train_dataloader),
                desc="Training",
            ):
                loss = run.training_step(batch, batch_idx, epoch)
            ...
            if (epoch >= (args.n_epochs * args.validation_skip)) or force:
                ...
                batch_idx = 0
                for i, valid_dataloader in enumerate(data.valid_dataloaders):
                    if isinstance(valid_dataloader.dataset, torch.utils.data.Subset):
                    ...
                    tqdm_desc = (
                        f"Validation {i+1}/" f"{len(data.valid_dataloaders)} ({camera})"
                    )
                    for batch in tqdm(
                        valid_dataloader, total=len(valid_dataloader), desc=tqdm_desc
                    ):
                ...
                do_create_checkpoint = checkpoint_helper.check_for_checkpoint(
                    logs=epoch_logs, add_checkpoint_metrics_to_logs=True
                )
                if epoch >= (args.n_epochs * args.checkpointing_skip) or force:
                    ...
                    for ckpt_metric in do_create_checkpoint:
                        ...
                        mapped_name = checkpoint_helper.metric_mapping_joined[
                            ckpt_metric
                        ]
                ...
                for key, value in examples.items():
                    ...
                    if isinstance(value, PIL.Image.Image):
                        value.save(os.path.join(examples_path, fn + ".png"), "PNG")
            ...
            if ((epoch % args.resume_ckpt_interval) == 0 and epoch > 0) or (
                ...
                # scheduler
                ckpt_filepath = os.path.join(checkpoints_path, f"ckpt_resume.pth")
        ...
        # store checkpoint
        ckpt_filepath = os.path.join(
            checkpoints_path, f"ckpt_error__epoch_{epoch:04d}.pth"
        )
        ckpt = {
            "state_dict": model.state_dict(),
            "optimizer": optimizer.state_dict(),
